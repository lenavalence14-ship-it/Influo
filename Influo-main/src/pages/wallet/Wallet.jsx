...
export default function Wallet() {
  ...
  return (
    <div className="px-5 pt-6 pb-6">
      ...
      <h1 className="text-h1 mb-6">Portefeuille</h1>

      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="glass-strong rounded-2xl p-4">
          <Unlock size={18} className="mb-2 text-[var(--text-secondary)]" />
          <p className="text-caption mb-1">Disponible</p>
          <p className="text-h1">{wallet.solde_disponible} €</p>
        </div>
        <div className="glass-strong rounded-2xl p-4">
          <Lock size={18} className="mb-2 text-[var(--text-secondary)]" />
          <p className="text-caption mb-1">Verrouillé</p>
          <p className="text-h1">{wallet.solde_verrouille} €</p>
        </div>
      </div>
      ...
      <div className="glass-strong rounded-2xl p-4 mb-6 flex items-center gap-3">
        ...
        <div>
          <p className="text-caption">Revenus totaux</p>
          <p className="text-h1">{wallet.revenus_totaux} €</p>
        </div>
      </div>
      ...
      {showAddMethod && (
        <div className="glass-strong rounded-2xl p-4 mb-4 space-y-3">
          <label className="block">
            <span className="block text-body mb-2 text-[var(--text-secondary)]">Opérateur</span>
            <select
              value={provider}
      ...
      <div className="space-y-2 mb-6">
        {transactions.length === 0 ? (
          <p className="text-caption">Aucune transaction.</p>
        ) : (
          transactions.map((t) => (
            <div key={t.id} className="glass rounded-2xl p-3 flex justify-between text-body">
      ...
      <div className="space-y-2">
        {retraits.length === 0 ? (
          <p className="text-caption">Aucun retrait.</p>
        ) : (
          retraits.map((r) => (
            <div key={r.id} className="glass rounded-2xl p-3 flex justify-between text-body">
