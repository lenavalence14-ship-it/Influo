...
export default function ChatPro() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'messages_pro', filter: `conversation_id=eq.${id}` }, (payload) => {
  ...
  const handleRequestPayment = async () => {
    ...
    if (editingCommande && commande?.id) {
      const { data: updated } = await supabase
        .from('commandes_pro')
        .update({ montant, commission, montant_net: montantNet, delai_livraison: paymentDelai || null })
        .eq('id', commande.id)
        .select()
        .single()
      setCommande(updated)
    ...
    const { data: newCommande } = await supabase
      ...
      .insert({
        ...
        delai_livraison: paymentDelai || null,
      })
      .select()
      .single()
    ...
    await sendSystemMessage(
      `💰 L'entreprise demande le paiement de ${montant} €${paymentDelai ? ` — délai de livraison : ${paymentDelai}` : ''}. (commande ${newCommande.numero_commande})`
    )
    setShowPaymentAsk(false)
    setPaymentAmount('')
    setPaymentDelai('')
  ...
  const handleDeleteCommande = async () => {
    ...
    await supabase.from('commandes_pro').delete().eq('id', commande.id)
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      <header className="flex items-center gap-3 px-3 py-2.5 sticky top-0 bg-[var(--bg-primary)]/90 backdrop-blur-xl z-20 border-b border-[var(--border)] shrink-0">
        ...
        {isEntreprise && (
          <div className="relative shrink-0">
            <button
              onClick={() => { setEditingCommande(false); setPaymentAmount(''); setPaymentDelai(''); setShowPaymentAsk(true) }}
      ...
      <div className="px-2 pb-[max(10px,env(safe-area-inset-bottom))] pt-1.5 flex items-center gap-1 shrink-0">
        ...
        {isEntreprise && commande?.status === 'paiement_demande' && (
          <div className="relative shrink-0">
            <button
              ...
              <div className="absolute bottom-full right-0 mb-2 glass-strong rounded-2xl overflow-hidden w-44 z-30">
                <button
                  onClick={() => {
                    ...
                    setPaymentDelai(commande.delai_livraison || '')
                    setShowCommandeMenu(false)
      ...
      {showPaymentAsk && (
        <BottomSheet
          ...
          title={editingCommande ? 'Modifier la demande' : 'Demander le paiement'}
        >
