...
export default function Chat() {
  ...
  const [editingCommande, setEditingCommande] = useState(false)
  const [showCommandeMenu, setShowCommandeMenu] = useState(false)
  ...
  const handleRequestPayment = async () => {
    ...
    // Édition d'une commande existante (pas encore payée) : on met à jour au lieu de
    // recréer, le numéro de commande (généré à la création) ne change pas.
    if (editingCommande && commande?.id) {
      const { data: updated } = await supabase
        .from('commandes')
        .update({ montant, commission, montant_net: montantNet })
        .eq('id', commande.id)
        .select()
        .single()
      setCommande(updated)
      await sendSystemMessage(`✏️ Demande de paiement modifiée : ${montant} € (commande ${updated.numero_commande}).`)
      setShowPaymentAsk(false)
      setPaymentAmount('')
      setEditingCommande(false)
      return
    }

    // Nouvelle commande : le numéro (000001-Influo, etc.) est généré automatiquement
    // par la base via une séquence globale, jamais côté front.
    ...
    await sendSystemMessage(
      `💰 L'influenceur demande le paiement de ${montant} € pour cette prestation. (commande ${newCommande.numero_commande})`
    )
    setShowPaymentAsk(false)
    setPaymentAmount('')
  }
  ...
  const handleDeleteCommande = async () => {
    ...
    if (!window.confirm(`Supprimer la demande de paiement ${commande.numero_commande} ?`)) return
    await supabase.from('commandes').delete().eq('id', commande.id)
    await sendSystemMessage(`🗑️ Demande de paiement ${commande.numero_commande} annulée.`)
    setCommande(null)
    setShowCommandeMenu(false)
  }
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <header className="flex items-center gap-3 px-3 py-2.5 sticky top-0 bg-[var(--bg-primary)]/90 backdrop-blur-xl z-20 border-b border-[var(--border)] shrink-0">
        ...
        {/* icône cash + tooltip permanent, influenceur uniquement, toujours visible */}
        {isInfluencer && (
          <div className="relative shrink-0">
            <button
              onClick={() => { setEditingCommande(false); setPaymentAmount(''); setShowPaymentAsk(true) }}
      ...
      <div className="px-2 pb-[max(10px,env(safe-area-inset-bottom))] pt-1.5 flex items-center gap-1 shrink-0">
        ...
        {/* menu 3-points : modifier / supprimer la demande de paiement, uniquement avant
            que l'argent ait bougé (status paiement_demande), et seulement pour celui qui l'a créée */}
        {isInfluencer && commande?.status === 'paiement_demande' && (
          <div className="relative shrink-0">
            <button
              onClick={() => setShowCommandeMenu((v) => !v)}
              aria-label="Options de la demande de paiement"
              className="w-9 h-9 flex items-center justify-center"
              style={{ color: 'var(--text-secondary)' }}
            >
              ⋮
            ...
            {showCommandeMenu && (
              <div className="absolute bottom-full right-0 mb-2 glass-strong rounded-2xl overflow-hidden w-44 z-30">
                <button
                  onClick={() => {
                    setEditingCommande(true)
                    setPaymentAmount(String(commande.montant))
                    setShowCommandeMenu(false)
                    setShowPaymentAsk(true)
                  }}
                  className="block w-full text-left px-4 py-3 text-body text-white hover:bg-white/10"
                >
                  Modifier
                </button>
                <button
                  onClick={handleDeleteCommande}
                  className="block w-full text-left px-4 py-3 text-body text-red-400 hover:bg-white/10"
                >
                  Supprimer
                </button>
              </div>
      ...
      {showPaymentAsk && (
        <BottomSheet
          ...
          title={editingCommande ? 'Modifier la demande' : 'Recevoir le paiement'}
        >
          <div className="px-4 pb-4 pt-1 space-y-2">
            {editingCommande && commande?.numero_commande && (
            ...
            <div className="flex gap-2">
              ...
              />
              <Button onClick={handleRequestPayment} disabled={!paymentAmount}>
                {editingCommande ? 'Enregistrer' : 'Demander'}
              </Button>
            </div>
            {!editingCommande && (
              <p className="text-caption text-[var(--text-secondary)] px-1">
                Un numéro de commande unique sera généré automatiquement.
              </p>
            )}
          </div>
        </BottomSheet>
