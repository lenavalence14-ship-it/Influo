...
export default function ChatBiz() {
  ...
  const handleRequestPayment = async () => {
    ...
    if (editingCommande && commande?.id) {
      const { data, error } = await supabase.rpc('edit_commande_biz', {
      ...
      setCommande(data)
      await sendSystemMessage(`✏️ Demande de paiement modifiée : ${data.montant} € (commande ${data.numero_commande}).`)
    ...
    await sendSystemMessage(
      `💰 Demande de paiement de ${data.montant} €${paymentDelai ? ` — délai de livraison : ${paymentDelai}` : ''}. (commande ${data.numero_commande})`
    )
    setShowPaymentAsk(false)
    setPaymentAmount('')
    setPaymentDelai('')
  ...
  const handleDeleteCommande = async () => {
    ...
    const { error } = await supabase.rpc('delete_commande_biz', { p_commande_id: commande.id })
    if (error) {
      setErrorMsg(error.message)
      return
    }
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      <header className="flex items-center gap-3 px-3 py-2.5 sticky top-0 bg-[var(--bg-primary)]/90 backdrop-blur-xl z-20 border-b border-[var(--border)] shrink-0">
        ...
        {canRequestPayment && (
          <div className="relative shrink-0">
            <button
              onClick={() => { setEditingCommande(false); setPaymentAmount(''); setPaymentDelai(''); setErrorMsg(''); setShowPaymentAsk(true) }}
      ...
      <div className="px-2 pb-[max(10px,env(safe-area-inset-bottom))] pt-1.5 flex items-center gap-1 shrink-0">
        ...
        {isDemandeur && commande?.status === 'paiement_demande' && (
          <div className="relative shrink-0">
            <button
      ...
      {showPaymentAsk && (
        <BottomSheet
          onClose={() => { setShowPaymentAsk(false); setErrorMsg(''); setEditingCommande(false) }}
