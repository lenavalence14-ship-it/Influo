...
export default function InfluencerProfile() {
  ...
  return (
    <div>
      ...
      {/* compteur dynamique : dépend de l'onglet et, dans "publications", du sous-onglet actif */}
      <div className="flex justify-center gap-1.5 py-2 text-caption text-[var(--text-secondary)] border-b border-[var(--border)]">
        {tab === 'publications' ? (
          subTab === 'video' ? (
            <span>
              <span className="text-[var(--text-primary)] font-medium">{collabVideoCount}</span>{' '}
              {collabVideoCount > 1 ? 'réels de collaboration' : 'réel de collaboration'}
            </span>
          ) : (
            <span>
              <span className="text-[var(--text-primary)] font-medium">{collabPhotoCount}</span>{' '}
              {collabPhotoCount > 1 ? 'photos de collaboration' : 'photo de collaboration'}
            </span>
          )
        ) : (
          <span>
